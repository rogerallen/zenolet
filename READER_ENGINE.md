...
4. **Chapter & Section Break Pagination (`alignChapterPageBreaks`)**:
   ...
   - **Table & Row Fragmentation**: Reader `table` elements allow fragmentation (`break-inside: auto;`) while individual rows preserve row integrity (`tr { break-inside: avoid; }`), preserving multi-cell tabular alignments (e.g. Roman numeral, title, and page number columns in illustration/content catalogs) on a single horizontal row.
